 ...
 * 4. Repeat until:
 *    A) No new cards appear after scrolls
  ...
  async scrape(url, limit = 0) {
    ...
    try {
      ...
      while (scrollCount < this.maxScrolls && noNewContentCount < this.noNewContentThreshold) {
        ...
        this.logger.debug(`[InfiniteScrollScraper] Scroll ${scrollCount + 1}: Found ${cardElements.length} total cards`);
        ...
        if (newCardsThisScroll === 0) {
          noNewContentCount++;
          this.logger.debug(`[InfiniteScrollScraper] No new contacts this scroll (${noNewContentCount}/${this.noNewContentThreshold})`);
        } else {
          noNewContentCount = 0;
          this.logger.info(`[InfiniteScrollScraper] Extracted ${newCardsThisScroll} new contacts this scroll`);
        }
