...
class SimpleScraper extends BaseScraper {
  ...
  /**
   * Scrape contacts using infinite scroll
   * @param {string} url - URL to scrape
   * @param {object} options - Scraping options
   * @param {string} options.cardSelector - CSS selector for cards
   * @param {number} options.maxItems - Maximum items to collect
   * @param {number} options.maxScrollAttempts - Maximum scroll attempts
   * @param {number} options.scrollDelay - Delay between scrolls (ms)
   * @returns {Promise<Array>} - Array of contacts
   */
  async scrapeInfiniteScroll(url, options = {}) {
    const {
      cardSelector = null,
      maxItems = Infinity,
    ...
    try {
      this.logger.info(`Starting infinite scroll scrape of ${url}`);
      const page = this.browserManager.getPage();
      ...
      // Detect card pattern if not provided
      const finalCardSelector = cardSelector || await this.detectCardPattern(page);
      this.logger.info(`Using selector: ${finalCardSelector || 'full page'}`);

      // Import InfiniteScrollHandler
      const { InfiniteScrollHandler } = require('../features/infinite-scroll');

      // Create handler
      const config = {
        cardSelector: finalCardSelector,
        infiniteScroll: {
          maxScrollAttempts,
          scrollDelay,
      ...
      const extractFn = async () => {
        const uniqueEmails = await this.extractUniqueEmails(page, finalCardSelector, null);
        const contacts = await this.buildContactsFromEmails(uniqueEmails, page, finalCardSelector);
        return contacts;
      };

      // Scroll and collect
      const allContacts = await handler.scrollAndCollect(extractFn, maxItems);

      // PDF fallback for missing names
      const contactsNeedingNames = allContacts.filter(c => !c.name);
      if (contactsNeedingNames.length > 0 && keepPdf) {
        this.logger.info(`Using PDF fallback for ${contactsNeedingNames.length} missing names...`);
        await this.fillNamesFromPdf(allContacts, page, keepPdf);
      }

      // Email-to-name extraction for remaining nulls
      const stillMissingNames = allContacts.filter(c => !c.name && c.email);
      if (stillMissingNames.length > 0) {
        this.logger.info(`Using email-to-name extraction for ${stillMissingNames.length} remaining contacts...`);
        for (const contact of stillMissingNames) {
          const derivedName = this.extractNameFromEmail(contact.email);
          if (derivedName) {
            contact.name = derivedName;
            contact.source = contact.source === 'html' ? 'html+email' : 'html+scroll+email';
          }
        }
      }

      this.logger.info(`Infinite scroll complete: ${allContacts.length} contacts extracted`);
      this.logger.info(`Scroll stats: ${JSON.stringify(handler.getStats())}`);
