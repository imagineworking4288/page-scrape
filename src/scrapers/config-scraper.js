 ...
 * Config Scraper v2.1
 *
 * Scrapes contacts using v2.0 and v2.1 configuration files generated by the
 * Visual Card Scraper. Uses card pattern matching and smart field extraction.
 *
 * Features:
 * - Loads and validates v2.0 and v2.1 config files
...
class ConfigScraper extends BaseScraper {
  ...
  async scrape(url, limit = 100, keep = false, pageNum = 1, sourceUrl = null) {
    ...
    try {
      ...
      let contacts;
      if (this.isV21 || this.config.version === '2.1') {
  ...
  /**
   * Get card selector from config (supports v2.0 and v2.1)
  ...
  /**
   * Get fallback card selectors (v2.1 only)
